...
Use Edit → Find (⌘F) to find or replace text. View → Larger Text / Smaller Text changes the editor font. Straight quotes are preserved; macOS smart-quote substitution is disabled for code.
...
:root{color-scheme:light dark;--bg:#faf9fc;--text:#252330;--muted:#767080;--line:#e4dfea;--code:#f0edf5;--accent:#7450a5}*{box-sizing:border-box}body{margin:0;font:13px/1.65 -apple-system,BlinkMacSystemFont,sans-serif;background:var(--bg);color:var(--text)}header{padding:20px 22px 14px;border-bottom:1px solid var(--line);position:sticky;top:0;background:var(--bg);z-index:2}.brand{font-weight:700;letter-spacing:.03em;color:var(--accent);margin-bottom:12px}input{width:100%;padding:9px 11px;border:1px solid var(--line);border-radius:8px;background:var(--code);color:var(--text);font:inherit}nav{display:flex;gap:6px;flex-wrap:wrap;padding:14px 22px;border-bottom:1px solid var(--line)}nav button{border:1px solid var(--line);border-radius:6px;background:transparent;color:var(--text);font:11px -apple-system,sans-serif;padding:5px 8px;cursor:pointer}nav button.active{background:var(--accent);border-color:var(--accent);color:white}main{padding:24px 22px 50px;max-width:920px;margin:auto}section{display:none}section.visible{display:block}h1{font-size:25px;line-height:1.2;font-weight:650;letter-spacing:-.5px;margin:6px 0 24px}h3{font-size:15px;line-height:1.35;margin:24px 0 10px}p{margin:8px 0 12px}.eyebrow{text-transform:uppercase;letter-spacing:1.4px;font-size:9px;color:var(--muted)}pre{position:relative;background:var(--code);border:1px solid var(--line);padding:14px;border-radius:9px;font:11px/1.7 ui-monospace,Menlo,monospace;white-space:pre;overflow:auto}code{font-family:ui-monospace,Menlo,monospace;font-size:.93em}p code{padding:2px 4px;background:var(--code);border-radius:3px}.bullet{padding-left:10px}.none{display:none;color:var(--muted)}.copy{float:right;border:0;background:var(--accent);color:white;border-radius:4px;font-size:10px;padding:3px 6px;cursor:pointer}section+section{margin-top:35px}footer{font-size:10px;color:var(--muted);border-top:1px solid var(--line);padding-top:18px;margin-top:30px}@media(prefers-color-scheme:dark){:root{--bg:#242229;--text:#e7e2ed;--muted:#a49aaa;--line:#403a48;--code:#302c37;--accent:#b99bdd}}
