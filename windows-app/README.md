...
- **FiMpp-Studio-Portable-1.2.2-x64.exe**: a single EXE that extracts its bundled runtime to a temporary directory and launches Studio without installation. Keep your letters in your own folder. Appearance preferences are saved in the Windows user profile.
- **FiMpp-Studio-Setup-1.2.2-x64.exe**: the installer, with destination selection, Start menu/Desktop shortcuts, `.fimpp`/`.fpp` file associations, and an uninstaller. Uninstalling preserves user settings and letters.
...
See [Windows signing setup](SIGNING.md). Official 1.2.2 builds use Azure Artifact Signing for the app, portable launcher, installer and uninstaller.
