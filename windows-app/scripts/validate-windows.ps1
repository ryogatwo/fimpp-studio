...
$stage = Join-Path $env:LOCALAPPDATA 'FiMStudio-Validation-1.2.4'
New-Item -ItemType Directory -Force $stage | Out-Null
$portable = Join-Path $stage 'FiMpp-Studio-Portable-1.2.4-x64.exe'
$installer = Join-Path $stage 'FiMpp-Studio-Setup-1.2.4-x64.exe'
Copy-Item (Join-Path $reports 'release\FiMpp-Studio-Portable-1.2.4-x64.exe') $portable -Force
Copy-Item (Join-Path $reports 'release\FiMpp-Studio-Setup-1.2.4-x64.exe') $installer -Force
