@echo off
setlocal
set "FIM_TEST_STAGE=%LOCALAPPDATA%\FiMStudio-Validation-1.2.5"
set "FIM_TEST_REPORTS=%~dp0..\build"
if not exist "%FIM_TEST_STAGE%" mkdir "%FIM_TEST_STAGE%"
copy /Y "%FIM_TEST_REPORTS%\release\FiMpp-Studio-Portable-1.2.5-x64.exe" "%FIM_TEST_STAGE%\" >nul
if errorlevel 1 exit /b 1
copy /Y "%FIM_TEST_REPORTS%\release\FiMpp-Studio-Setup-1.2.5-x64.exe" "%FIM_TEST_STAGE%\" >nul
if errorlevel 1 exit /b 1
echo Testing portable EXE...
start "" /wait "%FIM_TEST_STAGE%\FiMpp-Studio-Portable-1.2.5-x64.exe" --self-test --report="%FIM_TEST_STAGE%\windows-portable-validation.json"
...
start "" /wait "%FIM_TEST_STAGE%\FiMpp-Studio-Setup-1.2.5-x64.exe" /S /D=%FIM_TEST_STAGE%\Installed
