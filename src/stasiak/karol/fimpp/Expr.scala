...
case class Relational(left: Expr, op: String, right: Expr) extends Condition {
  def helper(e1: RuntimeValue, op: String, e2: RuntimeValue): Boolean = {
    (e1, op, e2) match {
      ...
      case (x, "=", y) => x == y
      case (x, "!=", y) => x != y
      case (RuntimeList(xs), op, y) if op.startsWith("all") => xs.forall(x => helper(x, op.substring(3), y))
      case (RuntimeList(xs), op, y) if op.startsWith("any") => xs.exists(x => helper(x, op.substring(3), y))
