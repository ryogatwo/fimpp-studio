...
object FimppParser extends RegexParsers {
  ...
  // FiM++ 1.0, Comments: P.S. / P.P.S. ignore text through the newline.
  // Also accept PS/PSS and P.S.S. spellings, case-insensitively.
  // Dotted markers need no following space; bare markers need a boundary.
  ...
  val keywords =
    Set("yes", "and", "got", "i", "my", "me", "with", "about", "I",
      ...
      "like", "when", "had", "was", "were", "in", "of", "on", "today", "made", "if", "from") //TODO
  val numbers = List("zero", "one", "two", "three", "four", "five", "six",
                     "seven", "eight", "nine", "ten", "eleven", "twelve")
  val ordinals = List(null, "first", "second", "third", "fourth", "fifth", "sixth",
  ...
  def kw(strs: String*): Parser[Unit] = strs match {
    case Seq() => failure(ERRORS.kwError: String)
    case Seq(str) =>
  ...
  def relationalCondition(ex: Parser[Expr]): Parser[Condition] = (
    ex ~ condOperator ~ ex ^^ { case e1 ~ op ~ e2 => Relational(e1, op, e2) }
      | altkw("everything", "everypony") ~> kw("in") ~> ex ~ condOperator ~ ex ^^
          { case e1 ~ op ~ e2 => Relational(e1, "all" + op, e2) }
      | altkw("anything", "anypony") ~> kw("in") ~> ex ~ condOperator ~ ex ^^
          { case e1 ~ op ~ e2 => Relational(e1, "any" + op, e2) }
      | ex <~ (isOrAre ~ altkw("nothing", "nopony")) ^^
          { e => Relational(e, "=", NullValue) }
      | ex <~ (isOrAre ~ altkw("something", "somepony")) ^^
          { e => Relational(e, "!=", NullValue) }
    )
  ...
  def increment: Parser[Increment] = positioned {
    (
      identifier ~ (kw("got")) ~ number ~ (kw("less") ^^^ (-1)
                      | kw("fewer") ^^^ (-1)
                      | kw("more") ^^^ 1) <~ sentenceEnd
        ^^ {
        case i ~ _ ~ n ~ dir =>
  ...
  def repeatStat: Parser[RepeatStat] = (
    (kw("i", "did", "this") ~> (opt(number <~ kw("times")
                                      | kw("once") ^^^ 1L) ^^ { ot => ot.getOrElse(1L) }) <~ headerEnd)
      ~ rep(statement)
      ~ opt(kw("it", "didn't", "work") ~ comma ~ kw("but", "i", "knew", "why") ~ headerEnd ~> rep(statement))
      ~ opt(kw("in", "the", "end") ~ comma ~ kw("i", "did", "this", "instead") ~ headerEnd ~> rep(statement))
      <~ kw("that's", "what", "i", "did") <~ sentenceEnd
  ...
  def globalDeclStat: Parser[GlobalDeclStat] = (
    kw("yes") ~> comma ~> opt(kw("i", "mean"))
                    ~> kw("that") ~> identifier <~ sentenceEnd ^^ { i => GlobalDeclStat(i) }
    )

  def callFunctionStat: Parser[ExprStat] = (
    kw("i", "also?") ~> altkw("did", "made", "caused")
                           ~> identifier ~ opt(kw("of") ~> arglist) <~ sentenceEnd ^^ {
  ...
  def callFunctionEachStat: Parser[ExprStat] = (
    kw("i", "also?") ~> altkw("did", "made", "caused")
                           ~> identifier ~ kw("of", "each", "of?") ~ expression <~ sentenceEnd ^^ {
  ...
  def classImportStat: Parser[Statement] = (kw("i", "enchanted")
                                            ~> identifier) ~ (kw("with") ~> rep(rawWord) <~ sentenceEnd) ^^ {
  ...
  def methodCallBranches: Parser[(Option[String], Option[String]) => MethodCall] = (
    (kw("about") ~> arglist) ~ (altkw("so", "what", "when", "if", "how")
                       ~> opt(altkw("they", "she", "he", "it")) ~> rawIdentifier) ^^ {
      case args ~ method => (a: Option[String], b: Option[String]) => MethodCall(a, b, method, args)
    }
      | (kw("about") ~> identifier) ~ (kw("made") ~> rawIdentifier) ^^ {
      case arg ~ method => (a: Option[String], b: Option[String])
                        => MethodCall(a, b, "set " + method, List(VariableValue(arg), BooleanValue(true)))
    }
      | (kw("about") ~> identifier) ~ (kw("with") ~> arglist) ~ (kw("of") ~> rawIdentifier) ^^ {
      case arg ~ args ~ method => (a: Option[String], b: Option[String])
                               => MethodCall(a, b, "set " + method, VariableValue(arg) :: args)
    }
      | (altkw("what", "if", "when") ~> identifier) ~ opt(kw("with")
                                   ~> arglist) ~ ((isOrAre | hasOrHave) ~> rawIdentifier) ^^ {
      case arg ~ args ~ method => (a: Option[String], b: Option[String])
                               => MethodCall(a, b, "? " + method, VariableValue(arg) :: (args.getOrElse(Nil)))
    }
    )
  ...
  def fieldGetStat: Parser[Statement] = (
    (kw("i") ~> altkw("took", "got", "stole") ~> simpleExpression)
      ~ (altkw("of","from") ~> identifier)    //added from -- JCM
  ...
  def commentStat: Parser[Statement] = ("""(?i)(by the way)\b""".r) ~ commentContent ~ sentenceEnd ^^^ NopStat

  // Parenthesized comments are handled as whitespace, without consuming a statement.

  def arrayAssignment: Parser[ArrayAssignment] = (
    (kw("on") ~> ordinalExpression)
      ~ (kw("of", "the?", "book?") ~> identifier)
      ~ (kw("i") ~> altkw("wrote", "scribbled", "noted")
                       ~> opt(kw("what", "i", "knew", "about") | kw("about")) ~> expression)
      <~ sentenceEnd
      ^^ {
  ...
  def arrayInit: Parser[ArrayInit] = (
    kw("today?", "i", "found", "a", "book") ~> opt("named" | "titled")
  ...
  def statement: Parser[Statement] = (
    assignment | arrayAssignment
      | callFunctionEachStat | callFunctionStat
      ...
      | functionCallEachAssign | functionCallAssign    // specific tuple syntax before generic calls
