...
import java.lang.reflect.Modifier.{PRIVATE, STATIC}
...
object JavaInterop {
  ...
  def normalizedMatch(fromFimNormalized: String, fromJavaNormalized: String): Boolean = {
    if (fromFimNormalized.charAt(0) == '?') "is" +
      fromFimNormalized.substring(1) == fromJavaNormalized || "get" +
      fromFimNormalized.substring(1) == fromJavaNormalized
  ...
  def callMethod(context: Context, clazz: Class[_],
                 methodName: String,
                 methodParams: List[RuntimeValue]): RuntimeValue = {
    ...
    clazz.getMethods.foreach { method =>
      ...
      if (!isPrivate(method.getModifiers)
        ...
        (if (isStatic(method.getModifiers)) methodParams.size else methodParams.size - 1)) {
        val score =
          (if (isStatic(method.getModifiers)) methodParams else methodParams.tail).zip(method.getParameterTypes).map {
          case (rv, cl) =>
            rv.matches(cl)
  ...
  def loadClass(id: List[String]): Class[_] = {
    ...
    def allPossibleShortClassNames(a: List[String]): List[String] = a match {
      ...
      case x :: xs =>
        ...
        if (x.length > 1 && x.length <= 3) {
          tmp = tmp ::: shorter.map(x.toUpperCase + _)
        }
        if (digits.contains(xl)) {
          tmp = tmp ::: shorter.map(digits.indexOf(xl) + _)
        }
        tmp
    }
    ...
    val classes = allPossibleClasses.distinct.filter {
      className =>
        try {
