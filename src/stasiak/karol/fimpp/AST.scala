...
case class FieldAssignment(objClass: String, field: Expr, value: Expr) extends Statement {
  def interpret(context: Context) {
    ...
    (oc,field) match {
      ...
      case (RuntimeClass(cl),StringValue(n)) => JavaInterop.setField(context, Left(cl), Right(n),v)
      case (RuntimeClass(cl),Concatenation(List(StringValue(n)))) => JavaInterop.setField(context, Left(cl), Right(n),v)
      ...
      case (RuntimeJavaObject(w),StringValue(n)) => JavaInterop.setField(context, Right(w), Right(n),v)
      case (RuntimeJavaObject(w),Concatenation(List(StringValue(n)))) => JavaInterop.setField(context, Right(w), Right(n),v)
...
case class WhileStat(cond: Condition, body:List[Statement],
                     catchBody: Option[List[Statement]],
                     finallyBody:List[Statement]) extends Statement{
  def interpret(context: Context) {
    catchBody match{
      case Some(cBody) =>
        try{
...
case class RepeatStat(times: Long, body:List[Statement],
                      catchBody: Option[List[Statement]],
                      finallyBody:List[Statement]) extends Statement{
  def interpret(context: Context) {
    catchBody match{
